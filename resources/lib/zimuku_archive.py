...
zimuku only use .rar, .zip and .7z (few) as compressing formats, so support for these 3 is enough for service.subtitle.zimuku.
...
def unpack(file_path):
    ...
    if file_path.endswith(self_archive_exts):
        ...
        subtitle_list = []
        log(sys._getframe().f_code.co_name,
            "Skip: Danger file ext: %s" % (archive_path),
            level=xbmc.LOGERROR)
